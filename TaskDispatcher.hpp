...
struct TaskDispatcher
{
	...
	void registerTask(std::function<void(std::unique_ptr<type_of_task> &)> handler)
	{
	...
	{
		std::unique_ptr<taskBase> tmp(std::move(task));
		const auto type_idx = std::type_index(typeid(type_of_task));
		functionMap[type_idx]->dispatch(tmp);
	}

private:
	std::map<std::type_index, std::unique_ptr<taskMappingBase>> functionMap;
};
