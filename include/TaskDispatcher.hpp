...
{
	TaskDispatcher(std::shared_ptr<executor::detail::ExecutorInterface> wg) : _wg(wg)
	{
	}
	...
	TokenPtr dispatch(std::shared_ptr<type_of_task>&& task)
	{
		...
		return _wg->dispatch([ptr, tmp = std::move(tmp)](int id) mutable { ptr->dispatch(tmp); });
	}

private:
	std::map<std::type_index, std::shared_ptr<taskMappingBase>> functionMap;
	std::shared_ptr<executor::detail::ExecutorInterface> _wg;
};
